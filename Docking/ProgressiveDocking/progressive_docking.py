...
parser.add_argument('-mn', '--model_number', required=False, default=-1)
...
# New way to track the model number
mn = int(io_args.model_number)
...
if transfer_learning and n_iteration > 1:
    from ML.DDMetrics import *
    # If we use transfer learning then load the model from the past iteration
    metrics = ['accuracy', tf.keras.metrics.Recall(), tf.keras.metrics.Precision()]
    progressive_docking = DDModel.load(SAVE_PATH + '/iteration_'+str(n_iteration-1)+'/all_models/model_'+str(mn),
                                       metrics=metrics, name="transfer_learning")

    # We next have to freeze layers and create a new top model
...
# If there was nothing passed for the model number, read the file
if mn == -1:
